...
async fn test_handle_request_with_mocked_spitter() {
    ...
    let response1 = handle_request(mock_spitter.clone(), request1).await.unwrap();
    assert_eq!(
        response1.into_body(),
        Body::from("Mocked spit response")
    );
    ...
    assert_eq!(
        response2.into_body(),
        Body::from("Mocked spit2 response")
    );
}
